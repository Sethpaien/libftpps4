...
#define DEBUG(format, ...)\
	do {\
		char buffer_dbg[512];\
		int size = sprintf(buffer_dbg, format, ##__VA_ARGS__);\
		sceNetSend(netdbg_sock, buffer_dbg, size, 0);\
	} while(0)

#define INFO(format, ...)\
	do {\
		char buffer_nfo[512];\
		int size = sprintf(buffer_nfo, format, ##__VA_ARGS__);\
		sceNetSend(netdbg_sock, buffer_nfo, size, 0);\
	} while(0)
...
static void cmd_PASV_func(ftpvita_client_info_t *client)
{
	...
	sprintf(data_socket_name, "FTPVita_client_%i_data_socket",client->num);
	...
	DEBUG("Data socket sceNetBind(): %d\n", ret);

	// Start listening
	ret = sceNetListen(client->data_sockfd, 128);
	DEBUG("Data socket sceNetListen(): %d\n", ret);
	...
	DEBUG("PASV mode port: %d (%d,%d)\n", picked.sin_port,picked.sin_port/256,picked.sin_port%256);
	...
	sprintf(cmd, "227 Entering Passive Mode (192,168,0,17,%d,%d)" FTPVITA_EOL,picked.sin_port/256,picked.sin_port%256);
		//(picked.sin_port >> 0) & 0xFF,
		//(picked.sin_port >> 8) & 0xFF);
...
static void cmd_PORT_func(ftpvita_client_info_t *client)
{
	...
	DEBUG("PORT connection to client(%d)'s IP: %s Port: %d Socket: %s\n",client->num, ip_str, data_port,data_socket_name);
...
static void client_open_data_connection(ftpvita_client_info_t *client)
{
	...
	//DEBUG("client_open_data_connection\n");
	if (client->data_con_type == FTP_DATA_CONNECTION_ACTIVE) {
		DEBUG("client_open_data_connection Connection Active\n");
		...
		DEBUG("client_open_data_connection Connection Passive (Socket %d, port %d)\n",client->data_sockfd,client->data_sockaddr.sin_port);
		...
		namelen = sizeof(picked);
		sceNetGetsockname(client->data_sockfd, (struct sockaddr *)&picked,&namelen);
		DEBUG("client_open_data_connection PASV mode port: %d (%d,%d)\n", picked.sin_port,picked.sin_port/256,picked.sin_port%256);
...
static int gen_list_format(char *out, int n, int dir,struct stat *stat ,const char *filename)
{
/*
...
*/	
	...
	time_t tmod;
	tmod = stat->st_mtim.tv_sec;
	modification = localtime(&tmod);
	
	DEBUG("gen_list_format: %s (%d)\n", filename,modification->tm_year);
	DEBUG("tmod: %d\n", tmod);
...
/*
	if  (2017 == modification->tm_year) {
		snprintf(yt, 6, "%02d:%02d", modification->tm_hour, modification->tm_min);
	}
	else {
		snprintf(yt, 6, "%04d", modification->tm_year);		
	}
*/
	//snprintf(yt, 6, "%04d", modification->tm_year);

	//snprintf(yt, 6, "%04d", 2017);

	return snprintf(out, n,"type=%s;size=%d;modify=%04d%02d%02d%02d%02d%02d; %s" FTPVITA_EOL,
			dir ? "dir" : "file",
			stat->st_size,
			1900+modification->tm_year,
			modification->tm_mon+1,
			modification->tm_mday,
			modification->tm_hour,
			modification->tm_min,
			modification->tm_sec,
			filename);
/*
	return snprintf(out, n,
		"%c%s 1 ps4 ps4 %u %s %-2d %04d %s" FTPVITA_EOL,
		dir ? 'd' : '-',
		dir ? "rwxr-xr-x" : "rw-r--r--",
		(unsigned int) stat->st_size,
		num_to_month[modification->tm_mon],
		modification->tm_mday,
		1900+modification->tm_year,
		filename);
*/
...
static void send_LIST(ftpvita_client_info_t *client, const char *path)
{
	...
	*/
	...
	if (send_devices) {
		DEBUG("Mountpoints Listing");
		//gen_list_format(buffer, sizeof(buffer), 1, &stat, "/");
		//gen_list_format(buffer, sizeof(buffer),	1, "/");
...
*/
		...
		while(getdents(dfd, buffer_dir, sizeof(buffer_dir)) != 0) {
			//DEBUG("Getdents.\n");
			dent = (struct dirent *)buffer_dir;

			while(dent->d_fileno) {
					strcpy(buffer_file,path);
					if (strcmp(path, "/") != 0)
						strcat(buffer_file,"/");
					strcat(buffer_file,dent->d_name);
					//DEBUG("Fullpath: %s\n", buffer_file);
					// Open for reading, and get size
					fd = open(buffer_file, O_RDONLY, 0);
					fstat(fd, &s);
					close(fd);
					//DEBUG("File fstat size %d.\n",s.st_size);
					//DEBUG("[%s]: %s\n", entryName(dent->d_type), dent->d_name);
					//DEBUG("%s\n", dent->d_name);
					if(dent->d_type==DT_DIR) gen_list_format(buffer, sizeof(buffer), 1,&s, dent->d_name);
					else gen_list_format(buffer, sizeof(buffer), 0,&s, dent->d_name);
					client_send_data_msg(client, buffer);
...
static int machine_list_format(char *out, int n, int dir,struct stat *stat ,const char *filename)
{	
	struct tm *modification;
	time_t tmod;
	tmod = stat->st_mtim.tv_sec;
...
static void send_MLSD(ftpvita_client_info_t *client, const char *path){
	...
	client_send_ctrl_msg(client, "150 Opening ASCII mode data transfer for MLSD." FTPVITA_EOL);
	...
	while(getdents(dfd, buffer_dir, sizeof(buffer_dir)) != 0) {
		...
		while(dent->d_fileno) {
				...
				if(dent->d_type==DT_DIR) machine_list_format(buffer, sizeof(buffer), 1,&s, dent->d_name);
				else machine_list_format(buffer, sizeof(buffer), 0,&s, dent->d_name);
				client_send_data_msg(client, buffer);
				memset(buffer, 0, sizeof(buffer));
				memset(buffer_file, 0, sizeof(buffer_file));
				dent = (struct dirent *)((void *)dent + dent->d_reclen);
...
static void cmd_MLSD_func(ftpvita_client_info_t *client)
{
	...
	if (list_cur_path)
		send_MLSD(client, client->cur_path);
	else
...
//	SceUID pd;
	...
	if (n < 1) {
		...
		DEBUG("Directory change : %s",client->cur_path);
	}
}
...
static void cmd_FEAT_func(ftpvita_client_info_t *client)
{
	...
	//client_send_ctrl_msg(client, "MLSD" FTPVITA_EOL);
...
{
	DEBUG("Client thread started! (argp %p)\n",argp);
	...
	DEBUG("Client thread %i started! (socket %08X)\n", client->num,client->ctrl_sockfd);

	client_send_ctrl_msg(client, "220 FTPPS4 Server ready." FTPVITA_EOL);
...
static void *server_thread(void *arg)
{
	...
	server_sockfd = sceNetSocket("FTPVita_server_sock",AF_INET,SOCK_STREAM,	0);

	DEBUG("Server socket fd: 0x%08X\n", server_sockfd);
	...
	DEBUG("Server sceNetBind(): %d\n", ret);

	// Start listening
	ret = sceNetListen(server_sockfd, 128);
	DEBUG("Server sceNetListen(): %d\n", ret);
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			...
			*/
			...
			//scePthreadJoin(client->thid, NULL);
...
int ftpvita_init(char *vita_ip, unsigned short int *vita_port)
{
	...
	server.sin_addr.s_addr = IP(192, 168, 0, 14);
	server.sin_port = sceNetHtons(9023);
	memset(server.sin_zero, 0, sizeof(server.sin_zero));

	netdbg_sock = sceNetSocket(socketName, AF_INET, SOCK_STREAM, 0);
	...
	if (ftp_initialized) {
		DEBUG("ftpvita_init() ftp initialized\n");
		return -1;
	}

	DEBUG("ftpvita_init() ftp not initialized\n");
	// Init Net 
...
*/
	...
	*vita_port = FTP_PORT;
	...
	sceNetInetPton(AF_INET, info.ip_address, &vita_addr);
	...
	*/
	//ScePthread thread;
	int foo = 0;
	scePthreadCreate(&server_thid, NULL, server_thread,  (void *)&foo, "FTPVita_server_thread");
